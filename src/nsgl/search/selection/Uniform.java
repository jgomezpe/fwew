...
public class Uniform<T> implements Selection<T>{
   ...
   */
  protected int choose_one( nsgl.integer.random.Uniform g ){
    return g.next();
  }
  ...
  public int choose_one( Tagged<T>[] x ){
    return choose_one(new nsgl.integer.random.Uniform(x.length));
  }
  ...
  public int[] apply( int n, Tagged<T>[] x){
	  nsgl.integer.random.Uniform g =  new nsgl.integer.random.Uniform(x.length);
    return g.generate(n);
  }
}