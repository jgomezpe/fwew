...
public abstract class GraphSearch<T,O> extends LocalSearch<T,Boolean>{
	...
	public abstract DynArray<O> apply( T initial, GraphSpace<T,O> space, Goal<T,Boolean> goal, OptionCost<T,O> cost  );
	...
	public Tagged<T> apply( Tagged<T> x, Space<T> space ){
		...
		DynArray<O> action = apply( x.unwrap(), g_space, goal, cost);
