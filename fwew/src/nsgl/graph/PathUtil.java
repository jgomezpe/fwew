...
public class PathUtil<T,O>{
  public double evaluate( T state, GraphSpace<T,O> space, DynArray<O> path, OptionCost<T,O> cost ){
    double c = 0.0;
    for(O action : path){
  ...
  public T final_state( T initial, GraphSpace<T,O> space, DynArray<O> path ){
      for(O action : path){
